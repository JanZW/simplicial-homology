...
It is possible to extend the concept of Homology to more general spaces than polyhedra using cellular or singular homology, however those will not be covered here. The interested reader might refer to the book of Munkres \cite{mu} or Hatcher \cite{ha} to learn more about either one.
...
When we concern ourselves with the question how closed paths on the torus are different from those on the sphere, we notice the following:
...
\parbox[t]{.5\linewidth}{\captionsetup{width=.9\linewidth}
\caption{There are closed paths on the torus that divide the surface into two regions ($B$) and those that don't ($A$).\label{path-on-torus}}}
...
\begin{center}
\begin{tikzcd}
	\cdots\arrow[r,"\partial"]&C_1(K)\arrow[r,"\partial"]\arrow[d,equals]&C_0(K)\arrow[d,equals]\arrow[r,"\partial"]&\arrow[d,equals]0\\
	\cdots\arrow[r,"\partial"]&0\arrow[r,"\partial"]&\Z\langle v\rangle\arrow[r,"\partial"]&0
\end{tikzcd}
\end{center}
