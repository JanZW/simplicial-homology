\begin{definition}
	Let $K$ be a simplicial complex. We say a chain $c\in C(K)$ is \textit{carried} by a subcomplex $L\subset K$ if $c$ vanishes on all simplices that are not contained in $L$. 
\end{definition}

The following lemma is taken from Munkres' book \parencite[Lemma 6.1, p. 34]{mu} and will be helpful for the next two examples.
\begin{lemma}%Lemma 6.1 Munkres
	Let $L$ be the complex depicted in \autoref{rectangle-tri}, whose underlying space is a rectangle. Let $\mathrm{Bd} L$ denote the complex whose underlying space is the boundary of the rectangle. Orient each 2-simplex $\sigma_i$ of $L$ counterclockwise. Orient the 1-simplices arbitrarily. Then:
	\begin{enumerate}
...
\begin{figure}\begin{center}
	\parbox{\linewidth}{	\parbox[c]{.6\linewidth}{\centering
	...
	\draw (2,0)--(6,4);
	\end{tikzpicture}	}%	
\parbox[c]{.4\linewidth}{\centering
	...
	\parbox{\linewidth}{
	\parbox[t]{.55\linewidth}{\caption{Complex whose underlying space is a rectangle.\label{rectangle-tri}}}\hspace{.05\linewidth}%
		\parbox[t]{.4\linewidth}{\caption{Structure of center. 2-simplices are oriented counterclockwise\label{zoom-in}}}}
\end{center}
\end{figure}


\begin{proof}
	We will prove the second statement fist. If $\sigma_i$ and $\sigma_j$ have an \glqq interior\grqq{} edge $e$ in common, then $\partial d$ hast to vanish on $e$. It follows that $d$ assignes the same value to $\sigma_i$ as it does to $\sigma_j$. %Continuing this process, we see that $d$ has the same value on every oriented 2-simplex $\sigma_i$.
	We use the same argument for all other $2$-simplices and find that $d$ assignes the same value to every oriented 2-simplex $\sigma_i$. 
	
	For the second part, let $c$ be be a 1-chain of $L$. We will \glqq push it off \grqq{} the 1-simplices in the following way:
	
	We concentrate on the center at first. There we notice the structure as depicted in \autoref{zoom-in}.
	
	

Let $c$ denote a 1-chain with value $a$ on $e_1$. Then $c$ is clearly homologous to the chain $c_1=c+\partial(a\sigma_1)$. The resulting 1-chain $c_1$ vanishes on $e_1$. We have \glqq pushed it off\grqq{} $e_1$. We can apply this multiple times for other edges until we arrive at a chain $c_2$ which is carried by the subcomplex depicted in \autoref{skelleton}.
\begin{figure}
	\begin{center}
\parbox{\linewidth}{		\parbox[c]{.5\linewidth}{
		\begin{tikzpicture}[yscale=0.5]
		\draw (0,0)--(6,0)--(6,6)--(0,6)--cycle;
		\draw (0,6)--(3,3);
		\draw (0,0)--(2,2);
...
\begin{proof}
	Let $g: |L|\to |T|$ be the pasting (gluing) map; let $A=g(|\mathrm{Bd} L|)$. Then $A$ is homeomorphic to\footnote{The wedge sum is the quotient of the disjoint union where two points are identified.\parencite[p. 10]{ha}} $\Sp^1\wedge \Sp^1$ as depicted in \autoref{wege-of-s1}. Orient the 1-simplices of $T$ arbitrarily. Since $g$ only identifies simplices in $\mathrm{Bd} L$, we find the same statements as above with identical arguments.
	\begin{enumerate}
...
Every 1-cycle of $T$ is homologous to a 1-cycle of the form $c=nw_1+mz_1$ by \ref{gen-1} and \ref{torus-1}. Such a cycle bounds only if it is trivial: If $c=\partial d$ for some $d$, then by virtue of \ref{gen-2} we have $d=p\gamma$ for some integer $p$. By \ref{torus-2} we know that $\partial\gamma=0$, so $c=0$. Therefore,
...
\begin{theorem}
	Let $S$ denote the simplicial complex represented by the labeled rectangle of \autoref{klein-tri}; Its underlying space is the Klein bottle. Then 
...
\end{figure}
\begin{proof}
	Let $g$ and $A$ be as before with the obvious changes. Again, $A$ is homeomorphic to $\Sp^1\wedge \Sp^1$. Orient all 2-simplices of $S$ as before and let $\gamma$ be their sum. Orient the 1-simplices arbitrarily. Once more, Statements \ref{gen-1} and \ref{gen-2} from the previous proof hold for the same reason as before. Since $A$ is the wedge sum of two circles, \ref{torus-1} holds again. However, condition \ref{torus-2} needs to be modified:
	...
	This result can be obtained via a direct calculation similar to what we did before.
	...
	For the second homology group, note that any 2-cycle $d$ of $S$ must be of the form $p\gamma$ by \ref{gen-2}. Further, \ref{klein-2} implies that $p\gamma$ is not a cycle, hence
	\[
	H_2(S)=0.
	\]
\end{proof}